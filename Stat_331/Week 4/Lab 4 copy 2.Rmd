...
house_price <- read_csv(here::here("Week 4","Sale_Prices_City.csv"))
...
#Challenge
I got the dataset from kaggle. The dataset in kaggle was sourced from Zillow and it contained the home prices for many cities in the US from 2011 through 2020.
link:https://www.kaggle.com/paultimothymooney/zillow-house-price-data
```{r avocado california houses}
CA_housing <- house_price %>% 
  filter(RegionName %in% c("Los Angeles", "San Diego", 
                       "Sacramento","San Francisco")) 

CA_housing <- CA_housing %>% 
  mutate(RegionName = if_else(RegionName == 'Los Angeles', 
                              'LosAngeles', RegionName))

CA_housing <- CA_housing %>% 
  mutate(RegionName = if_else(RegionName == 'San Diego', 
                              'SanDiego', RegionName))

CA_housing <- CA_housing %>% 
  mutate(RegionName = if_else(RegionName == 'San Francisco', 
                              'SanFrancisco', RegionName))

CA_housing <- CA_housing %>% 
  rename(region = RegionName)

housing_avocados <- CA_avocado %>% 
  inner_join(CA_housing, by = "region")

housing_avocados <- housing_avocados %>% 
  select(`2015-01`:`2018-03`, region, AveragePrice)

housing_avocados <- housing_avocados %>% 
  pivot_longer(`2015-01`:`2018-03`,
             names_to = 'Date', 
             values_to = 'HomePrice')

housing_avocados %>% 
  ggplot(mapping = aes(x = HomePrice, y = AveragePrice)) +
  geom_point() +
  facet_wrap(~ region)
